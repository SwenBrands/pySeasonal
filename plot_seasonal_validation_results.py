...
# variables = ['SPEI-3-R','SPEI-3-M','fwi'] #variable names in CDS format
...
if file_system == 'myLaptop':
    ...
    dir_figs = home+'/datos/tareas/proyectos/pticlima/seasonal/results/validation/'+vers #path to output figures file generated by this script
    ...
    dir_figs = home+'/Results/seasonal/validation/'+vers #path to output figures file generated by this script
    exec(open(rundir+'/functions_seasonal.py').read())
    ...
    for vv in np.arange(len(variables)):
        for dd in range(len(model_dataset)):
            ...
            for sc in np.arange(len(scores)):
                ...
                if scores[sc] in ('crps_ensemble_skillscore_clim'):
                    ...
                    savename = dir_figs+'/'+variables[vv]+'/pcolor_posgridboxes_'+variables[vv]+'_'+scores[sc]+'_'+domain+'_'+model_dataset[dd]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_testlvl_'+str(round(critval_rho*100))+'_'+vers+'.'+figformat
                    ...
                    savename = dir_figs+'/'+variables[vv]+'/pcolor_siggridboxes_'+variables[vv]+'_'+scores[sc]+'_'+domain+'_'+model_dataset[dd]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_testlvl_'+str(round(critval_rho*100))+'_'+vers+'.'+figformat
                ...
                #currently the 10th percentile of the cprs is assumed to set the binary mask to 1 (if below this percentile)
                    ...
                    savename = dir_figs+'/'+variables[vv]+'/pcolor_arealmean_'+variables[vv]+'_'+scores[sc]+'_'+domain+'_'+model_dataset[dd]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+vers+'.'+figformat
                ...
                if scores[sc] in ('relbias','bias') and detrending[det] == 'yes':
                    ...
                    for sea in np.arange(len(seasons)):
                        for ll in np.arange(len(leads)):
                            ...
                            savename = dir_figs+'/'+variables[vv]+'/maps/'+scores[sc]+'/map_'+variables[vv]+'_'+seasons[sea]+'_'+leads[ll]+'_'+scores[sc]+'_'+critval_label+'_'+domain+'_'+model_dataset[dd]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[0])+str(file_years[1])+'_'+vers+'.'+figformat
                            cbarlabel = scores[sc]
        ...
        for sc in np.arange(len(scores)):
            if scores[sc] in ('bias','relbias','pearson_r','spearman_r','crps_ensemble_skillscore_clim'):
                ...
                continue
        nc_results.close()
        savename_netcdf = dir_netcdf+'/binary_validation_results_pticlima_'+domain+'_'+str(file_years[0])+'_'+str(file_years[1])+'_'+vers+'.nc'
