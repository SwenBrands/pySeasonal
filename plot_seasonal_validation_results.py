...
if len(sys.argv) > 1:
    ...
    vers = 'v_test' #version of the validation results
...
# #set input parameters
# model = ['cmcc35'] # ['cmcc35','ecmwf51'] list of model or reanalysis datasets: era5 or era5_land
# corr_outlier = 'no' #load the outlier-correted validation results; yes or no
# detrending = ['no','yes'] #yes or no, linear detrending of the gcm and obs time series prior to validation
# file_years = [[1993,2022]] #[[1993,2022],[1981,2022]] list containing as many sub-lists as there are models to be validated; start and end years indicated in the input file name, [1982,2016] for mod2strong_Nino, [1984,2022] for mod2strong_Nina
...
variables = ['fwi','pvpot','SPEI-3-M','tp','ssrd','si10','t2m','msl'] #variable names in CDS format
variables_binary = ['fwi','pvpot','SPEI-3-M','tp','ssrd','si10','t2m','msl'] #variable names of the binary skill mask generated by this script; these are the variable names used by Predictia
...
# variables_binary = ['tp','ssrd','si10','t2m','msl'] #variable names of the binary skill mask generated by this script; these are the variable names used by Predictia
...
for ag in np.arange(len(agg_label)):
    ...
    for mm in range(len(model)):
        if model[mm] in ('ecmwf51','cmcc35','eccc5'):
            filename_input = 'validation_results_season_'+variables[0]+'_'+agg_label[ag]+'_'+model[mm]+'_vs_'+ref_dataset[0]+'_'+domain+'_corroutlier_'+corr_outlier+'_detrended_'+detrending[0]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[0]+'_'+vers+'.nc'
        else:
        ...
        for su in np.arange(len(subperiods)):
            for det in np.arange(len(detrending)):
                for vv in np.arange(len(variables)):
                    ...
                    filename_input = 'validation_results_season_'+variables[vv]+'_'+agg_label[ag]+'_'+model[mm]+'_vs_'+ref_dataset[vv]+'_'+domain+'_corroutlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+vers+'.nc'
                    nc_results = xr.open_dataset(dir_netcdf_scores+'/'+filename_input) #load the input dataset
                    
                    #optionally apply land sea mask; set values of sea to nan
                    if variables[vv] in masked_variables:
                        print('Upon user request, values for sea grid-boxes are set to nan for '+variables[vv]+' !')               
                        ...
                        nc_mask.close()
                        del(nc_mask)
                    elif variables[vv] not in masked_variables:
                        print('As requested by the user, the verification results are not filtered by a land-sea mask for '+variables[vv]+' !')
                    else:
                        ValueError('check whether <variables[vv]> is in <masked_variables> !')
                ...
                for vv in np.arange(len(variables)):
                    ...
                    for sc in np.arange(len(scores)):
                        ...
                        if scores[sc] in ('crps_ensemble_skillscore_clim','crps_ensemble_skillscore_rand','roc_auc_lower_tercile_skillscore','roc_auc_upper_tercile_skillscore'):
                            ...
                            savename_mean = dir_figs+'/'+subperiods[su]+'/'+variables[vv]+'/pcolor_arealmean_'+variables[vv]+'_'+agg_label[ag]+'_'+scores[sc]+'_'+sub_domain+'_'+model[mm]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+vers+'.'+figformat
                            savename_fraction = dir_figs+'/'+subperiods[su]+'/'+variables[vv]+'/pcolor_arealfraction_'+variables[vv]+'_'+agg_label[ag]+'_'+scores[sc]+'_'+sub_domain+'_'+model[mm]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+vers+'.'+figformat
