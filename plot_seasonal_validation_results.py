...
#set input parameters
model = ['eccc5'] # ['cmcc35','ecmwf51'] list of model or reanalysis datasets: era5 or era5_land
corr_outlier = 'no' #load the outlier-correted validation results; yes or no
detrending = ['no','yes'] #yes or no, linear detrending of the gcm and obs time series prior to validation
...
# subperiods = ['none'] # a list of strings defining the modulating oscillations and their phase, constructed from the <modulator> and <phase> input arguments in get_skill_season.py; currently "none", "enso0init", "enso1init" or "enso2init"

val_vers = 'v1m_test' #version of the validation results
...
for ag in np.arange(len(agg_label)):
    ...
    savename_netcdf = dir_netcdf_scores+'/binary_validation_results_pticlima_'+sub_domain+'_'+agg_label[ag]+'_'+val_vers+'.nc'
