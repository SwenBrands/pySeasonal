...
max_leads = []
for dd in range(len(model)):
    if model[dd] in ('ecmwf51','cmcc35'):
        filename_input = 'validation_results_season_'+variables[0]+'_'+model[dd]+'_vs_'+ref_dataset[0]+'_'+domain+'_corroutlier_'+corr_outlier+'_detrended_'+detrending[0]+'_'+str(file_years[dd][0])+'_'+str(file_years[dd][1])+'_'+subperiods[0]+'_'+vers+'.nc'
    else:
        raise Exception('ERROR: unknown entry for <model[dd]> !')
    ...
    for su in np.arange(len(subperiods)):
        for det in np.arange(len(detrending)):
            for vv in np.arange(len(variables)):
                ...
                filename_input = 'validation_results_season_'+variables[vv]+'_'+model[dd]+'_vs_'+ref_dataset[vv]+'_'+domain+'_corroutlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[dd][0])+'_'+str(file_years[dd][1])+'_'+subperiods[su]+'_'+vers+'.nc'
                nc_results = xr.open_dataset(dir_netcdf+'/'+filename_input) #load the input dataset
                ...
                for sc in np.arange(len(scores)):
                    if scores[sc] in ('bias','relbias'):
                        if scores[sc] == 'relbias' and variables[vv] in manual_cbar_variables:
                            ...
                            maxvals_map[su,det,vv,dd,sc] = np.abs(nc_results[scores[sc]]).max().values
                            minvals_map[su,det,vv,dd,sc] = np.abs(nc_results[scores[sc]]).max().values*-1
                            ...
                            maxvals_pcolor_mean[su,det,vv,dd,sc] = np.abs(meanvals_spatial).max()
                            minvals_pcolor_mean[su,det,vv,dd,sc] = np.abs(meanvals_spatial).max()*-1
                            maxvals_pcolor_fraction[su,det,vv,dd,sc] = fractions_spatial.max()
                            minvals_pcolor_fraction[su,det,vv,dd,sc] = 0
                        ...
                        maxvals_map[su,det,vv,dd,sc] = nc_results[scores[sc]].max().values
                        minvals_map[su,det,vv,dd,sc] = 0
                        meanvals_spatial = get_spatial_aggregation(nc_results[scores[sc]].values,mode_f='mean',lat_f=yy)
                        ...
                        maxvals_pcolor_mean[su,det,vv,dd,sc] = meanvals_spatial.max()
                        minvals_pcolor_mean[su,det,vv,dd,sc] = 0
                        maxvals_pcolor_fraction[su,det,vv,dd,sc] = fractions_spatial.max()
                        minvals_pcolor_fraction[su,det,vv,dd,sc] = 0
                        colormaps_fractions_spatial.append(colormap_ascend)
                        colormaps_meanvals_spatial.append(colormap_ascend)
                    elif scores[sc] in ('reliability_lower_tercile','reliability_upper_tercile'):
                        maxvals_map[su,det,vv,dd,sc] = 0.5 #global maximum for the specific model dataset and variable, suggested by Jose Manuel Gutiérrez
                        minvals_map[su,det,vv,dd,sc] = nc_results[scores[sc]].min().values #global minimum for the specific model dataset and variable
                        meanvals_spatial = get_spatial_aggregation(nc_results[scores[sc]].values,mode_f='mean',lat_f=yy)
                        ...
                        maxvals_pcolor_mean[su,det,vv,dd,sc] = meanvals_spatial.max()
                        minvals_pcolor_mean[su,det,vv,dd,sc] = meanvals_spatial.min()
                        #minvals_pcolor_mean[su,det,vv,dd,sc] = 0
                        maxvals_pcolor_fraction[su,det,vv,dd,sc] = fractions_spatial.max()
                        minvals_pcolor_fraction[su,det,vv,dd,sc] = 0
                        colormaps_fractions_spatial.append(colormap_ascend)
                        ...
                        if scores[sc] == 'rpc':
                            maxvals_map[su,det,vv,dd,sc] = 2
                            minvals_map[su,det,vv,dd,sc] = 0
                            maxvals_pcolor_mean[su,det,vv,dd,sc] = 2
                            minvals_pcolor_mean[su,det,vv,dd,sc] = 0
                        else:
                            maxvals_map[su,det,vv,dd,sc] = 1
                            minvals_map[su,det,vv,dd,sc] = -1
                            maxvals_pcolor_mean[su,det,vv,dd,sc] = np.abs(meanvals_spatial).max()
                            minvals_pcolor_mean[su,det,vv,dd,sc] = np.abs(meanvals_spatial).max()*-1
...
### get global min and max values covering the range of all considered model / reanalysis datasets, currently does not take into account various GCM datasets, i.e. only works for len(model) == 1
...
for dd in range(len(model)):
    dir_figs = home+'/Results/seasonal/validation/'+vers+'/'+sub_domain+'/'+model[dd]+'/'+str(file_years[dd][0])+'_'+str(file_years[dd][1]) #path to output figures file generated by this script
    for su in np.arange(len(subperiods)):
        for det in np.arange(len(detrending)):
            for vv in np.arange(len(variables)):
                ...
                #initializing output numpy matrix containing a binary resutls array (1 = significant skill, 0 = spurious skill)
                if su == 0 and det == 0 and vv == 0 and dd == 0:
                ...
                for sc in np.arange(len(scores)):
                    ...
                    if scores[sc] in ('crps_ensemble_skillscore_clim','crps_ensemble_skillscore_rand','roc_auc_lower_tercile_skillscore','roc_auc_upper_tercile_skillscore'):
                        ...
                        savename_mean = dir_figs+'/'+subperiods[su]+'/'+variables[vv]+'/pcolor_arealmean_'+variables[vv]+'_'+scores[sc]+'_'+sub_domain+'_'+model[dd]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[dd][0])+'_'+str(file_years[dd][1])+'_'+subperiods[su]+'_'+vers+'.'+figformat
                        savename_fraction = dir_figs+'/'+subperiods[su]+'/'+variables[vv]+'/pcolor_arealfraction_'+variables[vv]+'_'+scores[sc]+'_'+sub_domain+'_'+model[dd]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[dd][0])+'_'+str(file_years[dd][1])+'_'+subperiods[su]+'_'+vers+'.'+figformat
                        ...
                        savename_fraction = dir_figs+'/'+subperiods[su]+'/'+variables[vv]+'/pcolor_arealfraction_'+variables[vv]+'_'+scores[sc]+'_'+sub_domain+'_'+model[dd]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_testlvl_'+str(round(critval_rho*100))+'_'+str(file_years[dd][0])+'_'+str(file_years[dd][1])+'_'+subperiods[su]+'_'+vers+'.'+figformat
                        savename_mean = dir_figs+'/'+subperiods[su]+'/'+variables[vv]+'/pcolor_arealmean_'+variables[vv]+'_'+scores[sc]+'_'+sub_domain+'_'+model[dd]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_testlvl_'+str(round(critval_rho*100))+'_'+str(file_years[dd][0])+'_'+str(file_years[dd][1])+'_'+subperiods[su]+'_'+vers+'.'+figformat
                    ...
                    if scores[sc] in ('relbias','bias') and detrending[det] == 'yes':
                        ...
                        for sea in np.arange(len(seasons)):
                            for ll in np.arange(len(leads)):
                                ...
                                title = variables[vv]+' '+seasons[sea]+' '+leads[ll]+' '+scores[sc]+' '+critval_label+' dtr'+detrending[det]+' '+sub_domain+' '+model[dd]+' vs '+score_ref+' '+str(file_years[dd][0])+' '+str(file_years[dd][1])
                                savename = dir_figs+'/'+subperiods[su]+'/'+variables[vv]+'/maps/'+scores[sc]+'/map_'+variables[vv]+'_'+seasons[sea]+'_'+leads[ll]+'_'+scores[sc]+'_'+critval_label+'_'+sub_domain+'_'+model[dd]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[dd][0])+str(file_years[dd][1])+'_'+subperiods[su]+'_'+vers+'.'+figformat
                    ...
                    binary_mask[su,det,vv,dd,sc,:,0:lead_step,:,:] = binmask
                    #binary_mask[su,det,vv,dd,sc,:,:,:,:] = binmask
...
for sc in np.arange(len(scores)):
    if scores[sc] in ('rpc','bias','relbias','pearson_r','spearman_r','crps_ensemble_skillscore_clim','crps_ensemble_skillscore_rand','roc_auc_lower_tercile_skillscore','roc_auc_upper_tercile_skillscore'):
        ...
        ds_binary_mask.attrs['validation_period'] = str([dd][0])+' to '+str(file_years[dd][1])
...
nc_results.close()
#savename_netcdf = dir_netcdf+'/binary_validation_results_pticlima_'+domain+'_'+sub_domain+'_'+str(file_years[dd][0])+'_'+str(file_years[dd][1])+'_'+subperiods[su]+'_'+vers+'.nc'
savename_netcdf = dir_netcdf+'/binary_validation_results_pticlima_'+sub_domain+'_'+str(file_years[dd][0])+'_'+str(file_years[dd][1])+'_'+vers+'.nc'
