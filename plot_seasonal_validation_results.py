...
variables = ['tp','t2m'] #variable names in CDS format
variables_binary = ['pr','tas'] #variable names of the binary skill mask generated by this script; these are the variable names used by Predictia
...
for ag in np.arange(len(agg_label)):
    dir_netcdf_agg = dir_netcdf+'/'+agg_label[ag] #set path to the directory containing the validation results
    for mm in range(len(model)):
        ...
        nc_results = xr.open_dataset(dir_netcdf_agg+'/'+filename_input) #load the input dataset
...
for ag in np.arange(len(agg_label)):
    dir_netcdf_agg = dir_netcdf+'/'+agg_label[ag] #set path to the directory containing the validation results
    for mm in range(len(model)):
        dir_figs = dir_netcdf_agg+'/'+sub_domain+'/'+model[mm]+'/'+str(file_years[mm][0])+'_'+str(file_years[mm][1]) #path to output figures file generated by this script
        for su in np.arange(len(subperiods)):
            for det in np.arange(len(detrending)):
                for vv in np.arange(len(variables)):
                    ...
                    #load netcdf files containing the verification results
                    filename_input = 'validation_results_season_'+variables[vv]+'_'+agg_label[ag]+'_'+model[mm]+'_vs_'+ref_dataset[vv]+'_'+domain+'_corroutlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+val_vers+'.nc'
                    nc_results = xr.open_dataset(dir_netcdf_agg+'/'+filename_input)
    ...
    savename_netcdf = dir_netcdf_agg+'/binary_validation_results_pticlima_'+sub_domain+'_'+agg_label[ag]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+val_vers+'.nc'
