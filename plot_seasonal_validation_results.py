...
model = ['cmcc35','ecmwf51'] #list of model or reanalysis datasets: era5 or era5_land
...
file_years = [[1993,2022],[1981,2022]] #list containing as many sub-lists as there are models to be validated; start and end years indicated in the input file name, [1982,2016] for mod2strong_Nino, [1984,2022] for mod2strong_Nina
...
variables = ['tp','t2m','si10','ssrd','msl'] #variable names in CDS format
...
# variables = ['tp'] #variable names in CDS format
...
minvals_map = np.zeros((len(subperiods),len(detrending),len(variables),len(model),len(scores)))
minvals_map[:] = np.nan
...
for mm in range(len(model)):
    if model[mm] in ('ecmwf51','cmcc35'):
        filename_input = 'validation_results_season_'+variables[0]+'_'+model[mm]+'_vs_'+ref_dataset[0]+'_'+domain+'_corroutlier_'+corr_outlier+'_detrended_'+detrending[0]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[0]+'_'+vers+'.nc'
    else:
    ...
    max_leads = max_leads+[len(nc_results.lead.values)]
    nc_results.close()
    del(nc_results)
max_lead = np.max(max_leads) #size of the dimension describing the lead-time
print('The maximum length of the lead-time dimenions is '+str(max_lead)+' for '+str(model)+' !')
    ...
    for su in np.arange(len(subperiods)):
        for det in np.arange(len(detrending)):
            for vv in np.arange(len(variables)):
                ...
                filename_input = 'validation_results_season_'+variables[vv]+'_'+model[mm]+'_vs_'+ref_dataset[vv]+'_'+domain+'_corroutlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+vers+'.nc'
                nc_results = xr.open_dataset(dir_netcdf+'/'+filename_input) #load the input dataset
                ...
                for sc in np.arange(len(scores)):
                    if scores[sc] in ('bias','relbias'):
                        if scores[sc] == 'relbias' and variables[vv] in manual_cbar_variables:
                            ...
                            maxvals_map[su,det,vv,mm,sc] = np.abs(nc_results[scores[sc]]).max().values
                            minvals_map[su,det,vv,mm,sc] = np.abs(nc_results[scores[sc]]).max().values*-1
                            ...
                            maxvals_pcolor_mean[su,det,vv,mm,sc] = np.abs(meanvals_spatial).max()
                            minvals_pcolor_mean[su,det,vv,mm,sc] = np.abs(meanvals_spatial).max()*-1
                            maxvals_pcolor_fraction[su,det,vv,mm,sc] = fractions_spatial.max()
                            minvals_pcolor_fraction[su,det,vv,mm,sc] = 0
                        ...
                        maxvals_map[su,det,vv,mm,sc] = nc_results[scores[sc]].max().values
                        minvals_map[su,det,vv,mm,sc] = 0
                        meanvals_spatial = get_spatial_aggregation(nc_results[scores[sc]].values,mode_f='mean',lat_f=yy)
                        ...
                        maxvals_pcolor_mean[su,det,vv,mm,sc] = meanvals_spatial.max()
                        minvals_pcolor_mean[su,det,vv,mm,sc] = 0
                        maxvals_pcolor_fraction[su,det,vv,mm,sc] = fractions_spatial.max()
                        minvals_pcolor_fraction[su,det,vv,mm,sc] = 0
                        colormaps_fractions_spatial.append(colormap_ascend)
                        colormaps_meanvals_spatial.append(colormap_ascend)
                    elif scores[sc] in ('reliability_lower_tercile','reliability_upper_tercile'):
                        maxvals_map[su,det,vv,mm,sc] = 0.5 #global maximum for the specific model dataset and variable, suggested by Jose Manuel Gutiérrez
                        minvals_map[su,det,vv,mm,sc] = nc_results[scores[sc]].min().values #global minimum for the specific model dataset and variable
                        meanvals_spatial = get_spatial_aggregation(nc_results[scores[sc]].values,mode_f='mean',lat_f=yy)
                        ...
                        maxvals_pcolor_mean[su,det,vv,mm,sc] = meanvals_spatial.max()
                        minvals_pcolor_mean[su,det,vv,mm,sc] = meanvals_spatial.min()
                        #minvals_pcolor_mean[su,det,vv,mm,sc] = 0
                        maxvals_pcolor_fraction[su,det,vv,mm,sc] = fractions_spatial.max()
                        minvals_pcolor_fraction[su,det,vv,mm,sc] = 0
                        colormaps_fractions_spatial.append(colormap_ascend)
                        ...
                        if scores[sc] == 'rpc':
                            maxvals_map[su,det,vv,mm,sc] = 2
                            minvals_map[su,det,vv,mm,sc] = 0
                            maxvals_pcolor_mean[su,det,vv,mm,sc] = 2
                            minvals_pcolor_mean[su,det,vv,mm,sc] = 0
                        else:
                            maxvals_map[su,det,vv,mm,sc] = 1
                            minvals_map[su,det,vv,mm,sc] = -1
                            maxvals_pcolor_mean[su,det,vv,mm,sc] = np.abs(meanvals_spatial).max()
                            minvals_pcolor_mean[su,det,vv,mm,sc] = np.abs(meanvals_spatial).max()*-1
...
# minvals_pcolor_fraction = np.nanmin(np.reshape(minvals_pcolor_fraction,(minvals_pcolor_fraction.shape[0]*minvals_pcolor_fraction.shape[1]*minvals_pcolor_fraction.shape[2]*minvals_pcolor_fraction.shape[3],minvals_pcolor_fraction.shape[4])),axis=0)
# maxvals_pcolor_fraction = np.nanmax(np.reshape(maxvals_pcolor_fraction,(maxvals_pcolor_fraction.shape[0]*maxvals_pcolor_fraction.shape[1]*maxvals_pcolor_fraction.shape[2]*maxvals_pcolor_fraction.shape[3],maxvals_pcolor_fraction.shape[4])),axis=0)
# minvals_pcolor_mean = np.nanmin(np.reshape(minvals_pcolor_mean,(minvals_pcolor_mean.shape[0]*minvals_pcolor_mean.shape[1]*minvals_pcolor_mean.shape[2]*minvals_pcolor_mean.shape[3],minvals_pcolor_mean.shape[4])),axis=0)
# maxvals_pcolor_mean = np.nanmax(np.reshape(maxvals_pcolor_mean,(maxvals_pcolor_mean.shape[0]*maxvals_pcolor_mean.shape[1]*maxvals_pcolor_mean.shape[2]*maxvals_pcolor_mean.shape[3],maxvals_pcolor_mean.shape[4])),axis=0)

minvals_pcolor_fraction = xr.DataArray(minvals_pcolor_fraction,coords=[subperiods,detrending,variables,model,scores],dims=['subperiod', 'detrending','variable','model','score'], name='minimum_values')
maxvals_pcolor_fraction = xr.DataArray(maxvals_pcolor_fraction,coords=[subperiods,detrending,variables,model,scores],dims=['subperiod', 'detrending','variable','model','score'], name='maximum_values')

minvals_pcolor_mean = xr.DataArray(minvals_pcolor_mean,coords=[subperiods,detrending,variables,model,scores],dims=['subperiod', 'detrending','variable','model','score'], name='minimum_values')
maxvals_pcolor_mean = xr.DataArray(maxvals_pcolor_mean,coords=[subperiods,detrending,variables,model,scores],dims=['subperiod', 'detrending','variable','model','score'], name='maximum_values')

minvals_map = xr.DataArray(minvals_map,coords=[subperiods,detrending,variables,model,scores],dims=['subperiod', 'detrending','variable','model','score'], name='minimum_values')
maxvals_map = xr.DataArray(maxvals_map,coords=[subperiods,detrending,variables,model,scores],dims=['subperiod', 'detrending','variable','model','score'], name='maximum_values')

# minvals_map = np.nanmin(np.reshape(minvals_map,(minvals_map.shape[0]*minvals_map.shape[1]*minvals_map.shape[2]*minvals_map.shape[3],minvals_map.shape[4])),axis=0)
# maxvals_map = np.nanmax(np.reshape(maxvals_map,(maxvals_map.shape[0]*maxvals_map.shape[1]*maxvals_map.shape[2]*maxvals_map.shape[3],maxvals_map.shape[4])),axis=0)
# minvals_map = np.nanpercentile(np.reshape(minvals_map,(minvals_map.shape[0]*minvals_map.shape[1]*minvals_map.shape[2]*minvals_map.shape[3],minvals_map.shape[4])),1,axis=0)
# maxvals_map = np.nanpercentile(np.reshape(maxvals_map,(maxvals_map.shape[0]*maxvals_map.shape[1]*maxvals_map.shape[2]*maxvals_map.shape[3],maxvals_map.shape[4])),99,axis=0)

#then plot the results with this min and max values
for mm in range(len(model)):
    dir_figs = home+'/Results/seasonal/validation/'+vers+'/'+sub_domain+'/'+model[mm]+'/'+str(file_years[mm][0])+'_'+str(file_years[mm][1]) #path to output figures file generated by this script
    for su in np.arange(len(subperiods)):
        for det in np.arange(len(detrending)):
            for vv in np.arange(len(variables)):
                ...
                if su == 0 and det == 0 and vv == 0 and mm == 0:
                    ...
                    binary_mask = np.zeros((len(subperiods),len(detrending),len(variables),len(model),len(scores),len(nc_results.season),max_lead,len(y_coord),len(x_coord)),dtype='single')
                    binary_mask[:] = np.nan
                ...
                for sc in np.arange(len(scores)):
                    ...
                    if scores[sc] in ('crps_ensemble_skillscore_clim','crps_ensemble_skillscore_rand','roc_auc_lower_tercile_skillscore','roc_auc_upper_tercile_skillscore'):
                        ...
                        savename_mean = dir_figs+'/'+subperiods[su]+'/'+variables[vv]+'/pcolor_arealmean_'+variables[vv]+'_'+scores[sc]+'_'+sub_domain+'_'+model[mm]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+vers+'.'+figformat
                        savename_fraction = dir_figs+'/'+subperiods[su]+'/'+variables[vv]+'/pcolor_arealfraction_'+variables[vv]+'_'+scores[sc]+'_'+sub_domain+'_'+model[mm]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+vers+'.'+figformat
                        ...
                        savename_fraction = dir_figs+'/'+subperiods[su]+'/'+variables[vv]+'/pcolor_arealfraction_'+variables[vv]+'_'+scores[sc]+'_'+sub_domain+'_'+model[mm]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_testlvl_'+str(round(critval_rho*100))+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+vers+'.'+figformat
                        savename_mean = dir_figs+'/'+subperiods[su]+'/'+variables[vv]+'/pcolor_arealmean_'+variables[vv]+'_'+scores[sc]+'_'+sub_domain+'_'+model[mm]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_testlvl_'+str(round(critval_rho*100))+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+vers+'.'+figformat
                    ...
                    plot_pcolormesh_seasonal(pcolorme_mean,minvals_pcolor_mean.sel(variable=variables[vv],score=scores[sc]).min().values,maxvals_pcolor_mean.sel(variable=variables[vv],score=scores[sc]).max().values,savename_mean,colormaps_meanvals_spatial[sc],dpival)
                    plot_pcolormesh_seasonal(pcolorme_fraction,minvals_pcolor_fraction.sel(variable=variables[vv],score=scores[sc]).min().values,maxvals_pcolor_fraction.sel(variable=variables[vv],score=scores[sc]).max().values,savename_fraction,colormaps_fractions_spatial[sc],dpival)
                    ...
                    if scores[sc] in ('relbias','bias') and detrending[det] == 'yes':
                        ...
                        for sea in np.arange(len(seasons)):
                            for ll in np.arange(len(leads)):
                                ...
                                savename = dir_figs+'/'+subperiods[su]+'/'+variables[vv]+'/maps/'+scores[sc]+'/map_'+variables[vv]+'_'+seasons[sea]+'_'+leads[ll]+'_'+scores[sc]+'_'+critval_label+'_'+sub_domain+'_'+model[mm]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[mm][0])+str(file_years[mm][1])+'_'+subperiods[su]+'_'+vers+'.'+figformat
...
for sc in np.arange(len(scores)):
    if scores[sc] in ('rpc','bias','relbias','pearson_r','spearman_r','crps_ensemble_skillscore_clim','crps_ensemble_skillscore_rand','roc_auc_lower_tercile_skillscore','roc_auc_upper_tercile_skillscore'):
        ...
        ds_binary_mask.attrs['validation_period'] = str([mm][0])+' to '+str(file_years[mm][1])
...
nc_results.close()
#savename_netcdf = dir_netcdf+'/binary_validation_results_pticlima_'+domain+'_'+sub_domain+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+vers+'.nc'
savename_netcdf = dir_netcdf+'/binary_validation_results_pticlima_'+sub_domain+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+vers+'.nc'
