...
variables = ['tp','ssrd'] #variable names in CDS format
ref_dataset = ['era5','era5'] # #list of model or reference observational dataset paired with <variables> input parameter below
...
critval_bias = 0 # this is a dummy threshold so far, replace by p-value from a t-test for bias significantly distinct from zero in future versions
#scores = ['relbias','spearman_r','pearson_r','crps_ensemble_skillscore_clim']
scores = ['bias','spearman_r']
...
manual_cbar_variables = ('SPEI-3-R','SPEI-3-M','fwi') #variables not participating in the colorbar adjustment made by the script due to excessive relative biases, the cbar for these variables is set to range from <relbias_max>*-1 to <relbias_max>
meanrho_max = 1.
vers = 'v1j_mon'
...
figformat = 'png' #format of the output figures: pdf, png, etc.
        ...
        for dd in range(len(model_dataset)):
            ...
            for sc in np.arange(len(scores)):
                ...
                if scores[sc] in ('crps_ensemble_skillscore_clim'):
                    ...
                    pcolorme_meanrho = get_spatial_aggregation(rho.copy(),critval_rho,mode_f='mean',lat_f=yy) #.copy() is mandatory here; otherwise pval will be changed within the get_spatial_aggregation() function
                    units_pcolor = '%'
                ...
                #currently the 10th percentile of the cprs is assumed to set the binary mask to 1 (if below this percentile)
                    ...
                    pcolorme = get_spatial_aggregation(np.abs(nc_results[scores[sc]].values),critval_bias,mode_f='mean',lat_f=yy)
                    label_pcolor = 'areal mean absolute '+nc_results[scores[sc]].name
                    # #calculate areal mean bias
                    # pcolorme = get_spatial_aggregation(nc_results[scores[sc]].values,critval_bias,mode_f='mean',lat_f=yy)
                    # label_pcolor = 'areal mean '+nc_results[scores[sc]].name
                    ...
                    if scores[sc] == 'relbias':
                        info_string = 'relative bias of the ensemble mean time series in percent of the observed mean value exceeding '+str(critval_relbias)+', yes (1) or no (0)'
                        threshold = critval_relbias #this is a percentage threshold
                    elif scores[sc] == 'bias':
                        info_string = 'bias of the ensemble mean time series in percent of the observed mean value exceeding '+str(critval_bias)+', yes (1) or no (0)'
