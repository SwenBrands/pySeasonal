...
if len(sys.argv) > 1:
    ...
    vers = 'v1o' #version of the validation results

file_system = 'lustre' #lustre or myLaptop; used to create the path structure to the input and output files

#set input parameters
model = ['cmcc4','eccc5','cmcc35','ecmwf51'] # ['cmcc35','ecmwf51'] list of model or reanalysis datasets: era5 or era5_land
file_years = [[1993,2022],[1993,2022],[1993,2022],[1981,2022]] #[[1993,2022],[1981,2022]] list containing as many sub-lists as there are models to be validated; start and end years indicated in the input file name, [1982,2016] for mod2strong_Nino, [1984,2022] for mod2strong_Nina
variables = [['tp','ssrd','si10','t2m','msl'],['fwi','pvpot','SPEI-3-M','tp','ssrd','si10','t2m','msl'],['fwi','pvpot','SPEI-3-M','tp','ssrd','si10','t2m','msl'],['fwi','pvpot','SPEI-3-M','tp','ssrd','si10','t2m','msl']] #variable names in CDS format
variables_binary = [['tp','ssrd','si10','t2m','msl'],['fwi','pvpot','SPEI-3-M','tp','ssrd','si10','t2m','msl'],['fwi','pvpot','SPEI-3-M','tp','ssrd','si10','t2m','msl'],['fwi','pvpot','SPEI-3-M','tp','ssrd','si10','t2m','msl']] #variable names of the binary skill mask generated by this script; these are the variable names used by Predictia
ref_dataset = [['era5','era5','era5','era5','era5'],['era5','era5','era5','era5','era5','era5','era5','era5'],['era5','era5','era5','era5','era5','era5','era5','era5'],['era5','era5','era5','era5','era5','era5','era5','era5']] # #list of model or reference observational dataset paired with <variables> input parameter below
...
cont_scores = ['roc_auc_lower_tercile_skillscore','roc_auc_upper_tercile_skillscore'] #scores for which continuous values will be stored in the output files
...
for mm in np.arange(len(model)):
    if len(variables[mm]) != len(ref_dataset[mm]):
        raise Exception('ERROR: The two input lists <variables[mm]> and <ref_dataset[mm]> must have the same length !')
    if len(variables[mm]) != len(variables_binary[mm]):
        raise Exception('ERROR: The two input lists <variables[mm]> and <variables_binary[mm]> must have the same length !')
...
minvals_map = np.zeros((len(agg_label),len(subperiods),len(detrending),max_len_var,len(model),len(scores))) #init array containing zero values
...
for ag in np.arange(len(agg_label)):
    ...
    for mm in range(len(model)):
        if model[mm] in ('ecmwf51','cmcc35','cmcc4','eccc5'):
            filename_input = 'validation_results_season_'+variables[mm][0]+'_'+agg_label[ag]+'_'+model[mm]+'_vs_'+ref_dataset[mm][0]+'_'+domain+'_corroutlier_'+corr_outlier+'_detrended_'+detrending[0]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[0]+'_'+vers+'.nc'
        else:
        ...
        for su in np.arange(len(subperiods)):
            for det in np.arange(len(detrending)):
                for vv in np.arange(len(variables[mm])):
                    ...
                    filename_input = 'validation_results_season_'+variables[mm][vv]+'_'+agg_label[ag]+'_'+model[mm]+'_vs_'+ref_dataset[mm][vv]+'_'+domain+'_corroutlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+vers+'.nc'
                    nc_results = xr.open_dataset(dir_netcdf_scores+'/'+filename_input) #load the input dataset
...
minvals_pcolor_fraction = xr.DataArray(minvals_pcolor_fraction,coords=[agg_label,subperiods,detrending,variables[mm],model,scores],dims=['aggregation','subperiod', 'detrending','variable','model','score'], name='minimum_values')
maxvals_pcolor_fraction = xr.DataArray(maxvals_pcolor_fraction,coords=[agg_label,subperiods,detrending,variables[mm],model,scores],dims=['aggregation','subperiod', 'detrending','variable','model','score'], name='maximum_values')

minvals_pcolor_mean = xr.DataArray(minvals_pcolor_mean,coords=[agg_label,subperiods,detrending,variables[mm],model,scores],dims=['aggregation','subperiod', 'detrending','variable','model','score'], name='minimum_values')
maxvals_pcolor_mean = xr.DataArray(maxvals_pcolor_mean,coords=[agg_label,subperiods,detrending,variables[mm],model,scores],dims=['aggregation','subperiod', 'detrending','variable','model','score'], name='maximum_values')

minvals_map = xr.DataArray(minvals_map,coords=[agg_label,subperiods,detrending,variables[mm],model,scores],dims=['aggregation','subperiod', 'detrending','variable','model','score'], name='minimum_values')
maxvals_map = xr.DataArray(maxvals_map,coords=[agg_label,subperiods,detrending,variables[mm],model,scores],dims=['aggregation','subperiod', 'detrending','variable','model','score'], name='maximum_values')
...
for ag in np.arange(len(agg_label)):
            ...
            for det in np.arange(len(detrending)):
                for vv in np.arange(len(variables[mm])):
                    ...
                    if su == 0 and det == 0 and vv == 0 and mm == 0:
                        ...
                        #binary_mask = np.zeros((len(subperiods),len(detrending),len(variables[mm]),len(model),len(scores),len(nc_results.season),len(nc_results.lead),len(y_coord),len(x_coord)),dtype='single')
                        binary_mask = np.zeros((len(subperiods),len(detrending),max_len_var,len(model),len(scores),len(nc_results.season),int(lead_arr.sel(aggregation=agg_label[ag]).max().values),len(y_coord),len(x_coord)),dtype='single')
                        binary_mask[:] = np.nan
                        
                        # continuous_scores = np.copy(binary_mask)
                        # pdb.set_trace()
                    ...
                    for sc in np.arange(len(scores)):
                        ...
                        if scores[sc] in ('crps_ensemble_skillscore_clim','crps_ensemble_skillscore_rand','roc_auc_lower_tercile_skillscore','roc_auc_upper_tercile_skillscore'):
                            ...
                            savename_mean = dir_figs+'/'+subperiods[su]+'/'+variables[mm][vv]+'/pcolor_arealmean_'+variables[mm][vv]+'_'+agg_label[ag]+'_'+scores[sc]+'_'+sub_domain+'_'+model[mm]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+vers+'.'+figformat
                            savename_fraction = dir_figs+'/'+subperiods[su]+'/'+variables[mm][vv]+'/pcolor_arealfraction_'+variables[mm][vv]+'_'+agg_label[ag]+'_'+scores[sc]+'_'+sub_domain+'_'+model[mm]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+vers+'.'+figformat
                            ...
                            if scores[sc] in ('relbias','bias') and detrending[det] == 'yes':
                                ...
                                for sea in np.arange(len(seasons)):
                                    for ll in np.arange(len(leads)):
                                        ...
                                        title = variables[mm][vv]+' '+seasons[sea]+' '+leads[ll]+' '+scores[sc]+' '+critval_label+' dtr'+detrending[det]+' '+sub_domain+' '+model[mm]+' vs '+score_ref+' '+str(file_years[mm][0])+' '+str(file_years[mm][1])
    ...
    for sc in np.arange(len(scores)):
        if scores[sc] in ('rpc','bias','relbias','pearson_r','spearman_r','crps_ensemble_skillscore_clim','crps_ensemble_skillscore_rand','roc_auc_lower_tercile_skillscore','roc_auc_upper_tercile_skillscore'):
            ...
            binary_mask_score_i = xr.DataArray(binary_mask_score_i,coords=[subperiods,detrending,variables_binary[mm],model,nc_results.season,nc_results.lead,nc_results.y,nc_results.x],dims=['subperiod','detrended','variable', 'model', 'season', 'lead', 'y', 'x'], name=scores[sc]+'_binary')
            ...
            ds_binary_mask.attrs['validation_period'] = str(file_years)
    ...
    nc_results.close()
    # savename_netcdf = dir_netcdf+'/binary_validation_results_pticlima_'+domain+'_'+agg_label[ag]+'_'+sub_domain+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+vers+'.nc'
    # savename_netcdf = dir_netcdf_scores+'/binary_validation_results_pticlima_'+sub_domain+'_'+agg_label[ag]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+vers+'.nc'
    savename_netcdf = dir_netcdf_scores+'/binary_validation_results_pticlima_'+sub_domain+'_'+agg_label[ag]+'_'+vers+'.nc'
    ds_binary_mask.to_netcdf(savename_netcdf)
    ds_binary_mask.close()
    del(nc_results,ds_binary_mask)
...
# flagfile = dir_flag+'/plot_seasonal_valiations_results_'+str(vers)+'_'+str(model)+'_'+str(variables)+'_'+domain+'_'+sub_domain+'_'+str(agg_label)+'_'+str(subperiods)+'_mapping_'+plot_maps+'.flag'
flagfile = dir_flag+'/plot_seasonal_valiations_results_'+str(vers)+'_'+str(model)+'_'+domain+'_'+sub_domain+'_'+str(agg_label)+'_'+str(subperiods)+'_mapping_'+plot_maps+'.flag'
flagfile = flagfile.replace("[","").replace("]","").replace("'","").replace(",","_").replace(" ","")
file = open(flagfile,'w')
file.write('plot_seasonal_validation_results.py has been run successfully for '+str(vers)+', '+str(model)+', '+str(variables)+', '+str(domain)+', '+str(sub_domain)+', '+str(agg_label)+', '+str(subperiods)+', mapping '+plot_maps)
file.close()
