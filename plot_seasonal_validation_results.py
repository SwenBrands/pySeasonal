...
model = ['ecmwf51'] # ['cmcc35','ecmwf51'] list of model or reanalysis datasets: era5 or era5_land
...
file_years = [[1981,2022]] #[[1993,2022],[1981,2022]] list containing as many sub-lists as there are models to be validated; start and end years indicated in the input file name, [1982,2016] for mod2strong_Nino, [1984,2022] for mod2strong_Nina
...
subperiods = ['none']
val_vers = 'v1l_mon' #version of the validation results
...
variables = ['SPEI-3-M','tp','t2m'] #variable names in CDS format
variables_binary = ['SPEI-3-M','pr','tas'] #variable names of the binary skill mask generated by this script; these are the variable names used by Predictia
ref_dataset = ['era5','era5','era5'] # #list of model or reference observational dataset paired with <variables> input parameter below
...
for mm in range(len(model)):
    if model[mm] in ('ecmwf51','cmcc35'):
        filename_input = 'validation_results_season_'+variables[0]+'_'+model[mm]+'_vs_'+ref_dataset[0]+'_'+domain+'_corroutlier_'+corr_outlier+'_detrended_'+detrending[0]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[0]+'_'+val_vers+'.nc'
    else:
    ...
    for su in np.arange(len(subperiods)):
        for det in np.arange(len(detrending)):
            for vv in np.arange(len(variables)):
                ...
                filename_input = 'validation_results_season_'+variables[vv]+'_'+model[mm]+'_vs_'+ref_dataset[vv]+'_'+domain+'_corroutlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+val_vers+'.nc'
                nc_results = xr.open_dataset(dir_netcdf+'/'+filename_input) #load the input dataset
...
for mm in range(len(model)):
    dir_figs = home+'/Results/seasonal/validation/'+val_vers+'/'+sub_domain+'/'+model[mm]+'/'+str(file_years[mm][0])+'_'+str(file_years[mm][1]) #path to output figures file generated by this script
    for su in np.arange(len(subperiods)):
        for det in np.arange(len(detrending)):
            for vv in np.arange(len(variables)):
                ...
                for sc in np.arange(len(scores)):
                    ...
                    if scores[sc] in ('crps_ensemble_skillscore_clim','crps_ensemble_skillscore_rand','roc_auc_lower_tercile_skillscore','roc_auc_upper_tercile_skillscore'):
                        ...
                        savename_mean = dir_figs+'/'+subperiods[su]+'/'+variables[vv]+'/pcolor_arealmean_'+variables[vv]+'_'+scores[sc]+'_'+sub_domain+'_'+model[mm]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+val_vers+'.'+figformat
                        savename_fraction = dir_figs+'/'+subperiods[su]+'/'+variables[vv]+'/pcolor_arealfraction_'+variables[vv]+'_'+scores[sc]+'_'+sub_domain+'_'+model[mm]+'_vs_'+score_ref+'_corr_outlier_'+corr_outlier+'_detrended_'+detrending[det]+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+val_vers+'.'+figformat
...
nc_results.close()
#savename_netcdf = dir_netcdf+'/binary_validation_results_pticlima_'+domain+'_'+sub_domain+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+subperiods[su]+'_'+val_vers+'.nc'
savename_netcdf = dir_netcdf+'/binary_validation_results_pticlima_'+sub_domain+'_'+str(file_years[mm][0])+'_'+str(file_years[mm][1])+'_'+val_vers+'.nc'
