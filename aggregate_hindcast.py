        ...
        for yy in np.arange(len(years_vec)):
            ...
            for im in np.arange(len(imonth)):
                ...
                if (file_valid == 0) & (save_corrected_files == 'yes'):
                    ...
                    path_corrected_gcm_data = path_gcm_base_var+'/'+domain+'/'+product+'/'+variables[vv]+'/'+model[mm]+'/'+version[mm]+'/'+str(years_vec[yy])+str(imonth[im]).zfill(2)+'/corrected_seasonal-original-single-levels_'+domain+'_'+product+'_'+variables[vv]+'_'+model[mm]+'_'+version[mm]+'_'+str(years_vec[yy])+str(imonth[im]).zfill(2)+'.nc'
