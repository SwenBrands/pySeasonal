...
if len(sys.argv) > 1:
    ...
    elif variables_gcm[0] in ('GDD_S','GDD_W','CGDD_S','CGDD_W','pvpot','fwi','SPEI-3-M','t2m','tp','msl','si10','ssrd'):
    ...
    # variables_gcm = ['CGDD_W'] #model variable names in CDS format  GCM variable names have been set to ERA5 variable names from CDS in <aggregate_hindcast.py> except for <SPEI-3-M> and <SPEI-3-R>, which are paired with <SPEI-3> in <variables_obs>)
    # variables_obs = ['CGDD_W'] #variable names in observations; are identical to <variables_gcm> except for <SPEI-3>, which is referred to as <SPEI-3-M> or <SPEI-3-R> in the model depending on whether past values are taken from the model or reanalysis (i.e. quasi-observations)
    # agg_labels = ['1mon'] #list of strings used to defined the aggregation period in the output files generated by this script; the length of this list defines the number of aggregation windows
    # modulators = ['enso'] # list containing modulators climate oscillation assumed to modulate the verification results; currently: "enso" or "none"
