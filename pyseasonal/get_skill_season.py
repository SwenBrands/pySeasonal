...
if len(sys.argv) > 1:
    ...
    elif variables_gcm[0] in ('UAI','WSm','Tm','PVPOTm','pvpot','SU','DD','TR','FD','ID','pet-hargreaves','PRm','Rx1day','Rx5day','SSRDm','TNm','TXm','WSm','GDD_S','GDD_W','CGDD_S','CGDD_W','fwi','FWIm','tp','t2m','msl','si10','ssrd'):
    ...
    variables_gcm = ['TR'] #model variable names in CDS format  GCM variable names have been set to ERA5 variable names from CDS in <aggregate_hindcast.py> except for <SPEI-3-M> and <SPEI-3-R>, which are paired with <SPEI-3> in <variables_obs>)
    variables_obs = ['TR'] #variable names in observations; are identical to <variables_gcm> except for <SPEI-3>, which is referred to as <SPEI-3-M> or <SPEI-3-R> in the model depending on whether past values are taken from the model or reanalysis (i.e. quasi-observations)
