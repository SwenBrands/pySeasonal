...
if len(sys.argv) > 1:
    ...
    if variables_gcm[0] in ('SPEI-3-M-C4', 'SPEI-3-M', 'SPEI-3-R', 'SPEI-3-R-C1'):
        variables_obs = ['SPEI-3']
    elif variables_gcm[0] in ('CGDDS-C4', 'SPEI-3-M-C4', 'FWI-C4', 'Rx1day-C4', 'Rx5day-C4', 'TNm-C4', 'PRtot-C4', 'PRm-C4', 'TXm-C4', 'TXm-C6', 'FD-C4', 'SU-C4', 'TR-C4'):
        delete_me = variables_gcm[0][-3:]
        if delete_me in ('-C4','-C6'):
    ...
    elif variables_gcm[0] in ('CGDDS-C4_up010', 'FWI-C4_up010', 'SPEI-3-M-C4_up010', 'PRtot-C4_up010', 'PRm-C4_up010', 'TNm-C4_up010', 'TXm-C4_up010', 'FD-C4_up010', 'SU-C4_up010', 'TR-C4_up010', 'Rx1day-C4_up010', 'Rx5day-C4_up010'):
    ...
    elif variables_gcm[0] in ('UAI','WSm','Tm','PVPOTm','pvpot','SU','DD','TR','FD','ID','pet-hargreaves','PRm','Rx1day','Rx5day','SSRDm','TNm','TXm','WSm','GDD_S','GDD_W','CGDD_S','CGDD_W','GDD-S','GDD-W','CGDD-S','CGDD-W','fwi','FWIm','msl','tp','t2m','si10','ssrd'):
    ...
    # vers = 'v1s' #version number of the output netCDF file to be sent to Predictia; will be extended by "_mon" or "_seas" depending on whether 1-month or 3-month values will be verified
    # models = ['ecmwf51'] #['cmcc35','ecmwf51'] list of models and versions thereof
    # domain = 'Canarias' # string describing a single domain
    # variables_gcm = ['CGDDS-C4'] #model variable names in CDS format  GCM variable names have been set to ERA5 variable names from CDS in <aggregate_hindcast.py> except for <SPEI-3-M> and <SPEI-3-R>, which are paired with <SPEI-3> in <variables_obs>)
    # variables_obs = ['CGDDS'] #variable names in observations; are identical to <variables_gcm> except for <SPEI-3>, which is referred to as <SPEI-3-M> or <SPEI-3-R> in the model depending on whether past values are taken from the model or reanalysis (i.e. quasi-observations)
    ...
    # variables_gcm = ['TXm-C4_up010'] #model variable names in CDS format  GCM variable names have been set to ERA5 variable names from CDS in <aggregate_hindcast.py> except for <SPEI-3-M> and <SPEI-3-R>, which are paired with <SPEI-3> in <variables_obs>)
    # variables_obs = ['TXm_up010'] #variable names in observations; are identical to <variables_gcm> except for <SPEI-3>, which is referred to as <SPEI-3-M> or <SPEI-3-R> in the model depending on whether past values are taken from the model or reanalysis (i.e. quasi-observations)
    ...
    vers = 'v1t' #version number of the output netCDF file to be sent to Predictia; will be extended by "_mon" or "_seas" depending on whether 1-month or 3-month values will be verified
    models = ['dwd22'] #['cmcc35','ecmwf51'] list of models and versions thereof
    domain = 'medcof' # string describing a single domain
    variables_gcm = ['ssrd'] #model variable names in CDS format  GCM variable names have been set to ERA5 variable names from CDS in <aggregate_hindcast.py> except for <SPEI-3-M> and <SPEI-3-R>, which are paired with <SPEI-3> in <variables_obs>)
    variables_obs = ['ssrd'] #variable names in observations; are identical to <variables_gcm> except for <SPEI-3>, which is referred to as <SPEI-3-M> or <SPEI-3-R> in the model depending on whether past values are taken from the model or reanalysis (i.e. quasi-observations)
    agg_labels = ['5mon'] #list of strings used to defined the aggregation period in the output files generated by this script; the length of this list defines the number of aggregation windows
    modulators = ['none'] # list containing modulators climate oscillation assumed to modulate the verification results; currently: "enso" or "none"
    phases = ['none'] #changed to string format for all entries in the new version
    FLAGDIR = '/lustre/gmeteo/PTICLIMA/Scripts/SBrands/pyPTIclima/pySeasonal/FLAG/get_skill/'+domain
...
#set path to the land-sea mask that can be applied optionally
#get mask label as a function of the requested domain
    ...
    for ag in np.arange(len(agg_labels)):
        ...
        for mm in np.arange(len(models)):
            for det in np.arange(len(detrending)):
                for vv in np.arange(len(variables_gcm)):
                    ...
                    for oo in np.arange(len(obs)):
                        path_obs = path_obs_base+'/'+obs[oo]+'/'+variables_obs[vv]+'_'+aggreg+'_'+obs[oo]+'_on_'+grid_name+'_grid_'+domain+'_'+str(years_obs[0])+'_'+str(years_obs[-1])+'.nc'
                        ...
                        start_time_quantiles = time.time()
                        ...
                        end_time_quantiles = time.time()
                        elapsed_time_quantiles = (end_time_quantiles - start_time_quantiles)/60
                        print('The execution time for caclulating the modelled and observed QUANTILES was: '+str(elapsed_time_quantiles)+' minutes')
                        ...
                        for ll in np.arange(len(lead_label)):
                            ...
                            roc_auc_lower = get_reliability_or_roc(obs_seas_mn_5d.isel(time=phase_ind).sel(lead=lead_label[ll]),gcm_seas_mn_6d.isel(time=phase_ind).sel(lead=lead_label[ll]),obs_quantile_vals_step.sel(lead=lead_label[ll]),gcm_quantile_vals_step.sel(lead=lead_label[ll]),'lower_tercile',score_f = 'roc_auc', bin_edges_f=prob_thresholds).rename('roc_auc_lower_tercile') #calculate roc area under the curve for the first tercile
                            roc_auc_lower_np[:,ll,:,:] = roc_auc_lower.values
                            roc_auc_lower.close()
                            del(roc_auc_lower)

                            roc_auc_center = get_reliability_or_roc(obs_seas_mn_5d.isel(time=phase_ind).sel(lead=lead_label[ll]),gcm_seas_mn_6d.isel(time=phase_ind).sel(lead=lead_label[ll]),obs_quantile_vals_step.sel(lead=lead_label[ll]),gcm_quantile_vals_step.sel(lead=lead_label[ll]),'center_tercile',score_f = 'roc_auc', bin_edges_f=prob_thresholds).rename('roc_auc_center_tercile') #calculate roc area under the curve for the third tercile
                            roc_auc_center_np[:,ll,:,:] = roc_auc_center.values
                            roc_auc_center.close()
                            del(roc_auc_center)

                            roc_auc_upper = get_reliability_or_roc(obs_seas_mn_5d.isel(time=phase_ind).sel(lead=lead_label[ll]),gcm_seas_mn_6d.isel(time=phase_ind).sel(lead=lead_label[ll]),obs_quantile_vals_step.sel(lead=lead_label[ll]),gcm_quantile_vals_step.sel(lead=lead_label[ll]),'upper_tercile',score_f = 'roc_auc', bin_edges_f=prob_thresholds).rename('roc_auc_upper_tercile') #calculate roc area under the curve for the third tercile
                        ...
                        #apply land sea-mask
                        if variables_gcm[vv] in masked_variables_std:
                            results = apply_sea_mask(results,mask_file,'y','x')
            ...
            if modulators[mo] == 'none':
                ...
                ## interpolate tercile probability values to observational PTI grid
                ...
                if variables_gcm[vv] in masked_variables_std and mask_probabilities == 'yes': # <masked_variables_std> is defined in ./config/config_for_get_skill_season.py
                    print('Upon user request set in '+configuration_file+', values above the Sea are set to NAN for '+variables_gcm[vv]+' in the xarray dataset containing modelled tercile probability time-series ! ')
                    tercile_prob = apply_sea_mask(tercile_prob,mask_file,'y','x')
                else:
                    print('Upon user request set in ./config/config_for_get_skill_season.py, values above the Sea are RETAINED for '+variables_gcm[vv]+' in the xarray dataset containing modelled tercile probability time-series ! ')
                ...
                if variables_gcm[vv] in masked_variables_std: # <masked_variables_std> is defined in ./config/config_for_get_skill_season.py
                    print('Upon user request set in ./config/config_for_get_skill_season.py, values above the Sea are set to NAN for '+variables_obs[vv]+' in the xr dataset containing observed tercile occurrence time-series ! ')
                    tercile_bin_obs = apply_sea_mask(tercile_bin_obs,mask_file,'y','x') #mask_file is defined above
                else:
                    print('Upon user request set in ./config/config_for_get_skill_season.py, values above the Sea are RETAINED for '+variables_obs[vv]+' in the xr dataset containing observed tercile occurrence time-series ! ')
