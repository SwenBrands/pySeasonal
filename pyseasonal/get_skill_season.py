...
if len(sys.argv) > 1:
    ...
    elif variables_gcm[0] in ('PRtot-C4_up010', 'PRm-C4_up010', 'TNm-C4_up010', 'TXm-C4_up010', 'FD-C4_up010', 'SU-C4_up010', 'TR-C4_up010', 'Rx5day-C4_up010'):
    ...
    variables_gcm = ['PRm-C4'] #model variable names in CDS format  GCM variable names have been set to ERA5 variable names from CDS in <aggregate_hindcast.py> except for <SPEI-3-M> and <SPEI-3-R>, which are paired with <SPEI-3> in <variables_obs>)
    variables_obs = ['PRm'] #variable names in observations; are identical to <variables_gcm> except for <SPEI-3>, which is referred to as <SPEI-3-M> or <SPEI-3-R> in the model depending on whether past values are taken from the model or reanalysis (i.e. quasi-observations)
            ...
            for det in np.arange(len(detrending)):
                for vv in np.arange(len(variables_gcm)):
                    ...
                    for oo in np.arange(len(obs)):
                        ...
                        center_tercile_obs_step = ((obs_seas_mn_5d > t1_obs) and (obs_seas_mn_5d <= t2_obs)).astype(precision)
                        ...
                        if modulators[mo] == 'none':
                            ...
                            valid_ind = ~np.isnan(lower_np) and ~np.isnan(upper_np)
                            upper_ind = (gcm_seas_mn_6d > upper_np) and valid_ind
                            center_ind = (gcm_seas_mn_6d > lower_np) and (gcm_seas_mn_6d <= upper_np) and valid_ind
                            lower_ind = (gcm_seas_mn_6d <= lower_np) and valid_ind
