...
def swen_seas2ipe(config: dict, year_init: str, month_init: str):
    ...
    for ag in np.arange(len(agg_labels)):
        for mm in np.arange(len(model)):
            ...
            savename = dir_output+'/seas2ipe_singlevars_'+model[mm]+version[mm]+'_'+domain+'_init_'+str(year_init)+str(month_init).zfill(2)+'_'+str(fc_season_length)+'mon_dtr_'+detrended+'_refyears_'+str(tercile_attrs['tercile_period'][0])+'_'+str(tercile_attrs['tercile_period'][1])+'.nc'
