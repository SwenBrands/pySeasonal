...
if len(sys.argv) > 1:
    ...
    agg_label = ['1mon']
    vers = 'v1s' #version of the validation results
    domain = 'medcof'


# INDICATE CONFIGURATION FILE ######################################

configuration_file = 'config/config_for_plot_seasonal_validation_results_'+domain+'.yaml'
...
critval_skillscore = config['critval_skillscore'] #used for crpss and rpc; is currently set to 0 irrespective of the sample size
            ...
            for su in np.arange(len(subperiods)):
                for det in np.arange(len(detrending)):
                        ...
                        if su == 0 and det == 0 and vv == 0:
                            ...
                            #pdb.set_trace()
                            binary_mask = np.zeros((len(subperiods),len(detrending),len(scores),len(nc_results.season),int(lead_arr.sel(aggregation=agg_label[ag],model=models[mm]).max().values),len(y_coord),len(x_coord)),dtype='single')
                            binary_mask[:] = np.nan
                            continuous_score = np.zeros((len(subperiods),len(detrending),len(cont_scores),len(nc_results.season),int(lead_arr.sel(aggregation=agg_label[ag],model=models[mm]).max().values),len(y_coord),len(x_coord)),dtype='single')
                        ...
                        for sc in np.arange(len(scores)):
                            ...
                            if scores[sc] in ('crps_ensemble_skillscore_clim','crps_ensemble_skillscore_rand','roc_auc_lower_tercile_skillscore','roc_auc_center_tercile_skillscore','roc_auc_upper_tercile_skillscore'):
                                ...
                                if scores[sc] in ('crps_ensemble_skillscore_clim','crps_ensemble_skillscore_rand'):
                                    ...
                                    label_pcolor_fraction = 'areal fraction > '+str(round(critval_skillscore,2))+' '+scores[sc]
                                elif scores[sc] in ('rpc'):
                                ...
                                if scores[sc] == 'crps_ensemble_skillscore_clim':
                                    ...
                                    score_info[sc] = 'ROC Area Under the Curve skill score with reference to a random forecast exceeding '+str(critval_rocss)+', yes (1) or no (0)'
                                else:
                                ...
                                label_pcolor_fraction = 'areal fraction with '+nc_results[scores[sc]].name+' < '+str(round(critval_rpc,2))
                                ...
                                if scores[sc] in ('relbias','bias') and detrending[det] == 'yes':
                                    ...
                                    for sea in np.arange(len(seasons)):
                                        for ll in np.arange(len(leads)):
                                            ...
                                            elif scores[sc] in ('crps_ensemble_skillscore_clim','crps_ensemble_skillscore_rand'):
                ...
                for sc in np.arange(len(scores)):
                    if scores[sc] in ('rpc','bias','relbias','pearson_r','spearman_r','crps_ensemble_skillscore_clim','crps_ensemble_skillscore_rand','roc_auc_lower_tercile_skillscore','roc_auc_center_tercile_skillscore','roc_auc_upper_tercile_skillscore'):
                        ...
                        binary_mask_score_i = np.expand_dims(binary_mask_score_i, axis = 0)
                        # convert numpy array to xarray data array
                        binary_mask_score_i = xr.DataArray(binary_mask_score_i,coords=[[variables_out[mm][vv]], subperiods,detrending,nc_results.season,nc_results.lead,nc_results.y,nc_results.x],dims=['variable', 'subperiod', 'detrended', 'season', 'lead', 'y', 'x'], name=scores[sc]+'_binary')
                        
                        # add singleton dimension "variable" as requested by Daniel Garcia
                ...
                for csc in np.arange(len(cont_scores)):
                    ...
                    continuous_score_i = np.expand_dims(continuous_score_i, axis = 0)
                    # convert numpy array to xarray data array
                    continuous_score_i = xr.DataArray(continuous_score_i,coords=[[variables_out[mm][vv]],subperiods,detrending,nc_results.season,nc_results.lead,nc_results.y,nc_results.x],dims=['variable', 'subperiod', 'detrended', 'season', 'lead', 'y', 'x'], name=cont_scores[csc]+'_continuous')
                ...
                #clean
                ds_binary_mask; ds_continuous_score
                del(ds_binary_mask, ds_continuous_score)
                ...
                nc_results.close()
                savename_netcdf = dir_netcdf_skillmasks+'/skill_masks_pticlima_'+sub_domain+'_'+agg_label[ag]+'_'+models[mm]+'_'+variables_out[mm][vv]+'_'+vers+'.nc'
                ...
                del(nc_results,ds_mask_plus_cont,chunks,encoding)
