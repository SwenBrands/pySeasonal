...
variables = [['pvpot','fwi','SPEI-3-M','pr','tas','psl','sfcWind','rsds'],['pvpot','fwi','SPEI-3-M','pr','tas','psl','sfcWind','rsds']] #variable names in directories and file names
variables_nc = [['pvpot','FWI','SPEI-3-M','pr','tas','psl','sfcWind','rsds'],['pvpot','FWI','SPEI-3-M','pr','tas','psl','sfcWind','rsds']] #variable names within the netCDF files, differs in case of fwi msi (msi / FWI is used within the file, but psl / fwi is used in the file name)
variables_new = [['pvpot','fwi','SPEI-3-M','tp','t2m','msl','si10','ssrd'],['pvpot','fwi','SPEI-3-M','tp','t2m','msl','si10','ssrd']] #new variable names; as provided by ERA5 data from CDS
time_name = [['time','time','time','forecast_time','forecast_time','forecast_time','forecast_time','forecast_time'],['time','time','time','forecast_time','forecast_time','forecast_time','forecast_time','forecast_time']] #list containing lists, each sublist specifies the names of the time coordinates for each variables of a specific model; one list per model
lon_name = [['lon','lon','lon','x','x','x','x','x'],['lon','lon','lon','x','x','x','x','x']] #as time_name but for the name of the longitude coordinate
lat_name = [['lat','lat','lat','y','y','y','y','y'],['lat','lat','lat','y','y','y','y','y']] #as time_name but for the name of the latitude coordinate
file_start = [['seasonal-original-single-levels_derived','seasonal-original-single-levels_derived','seasonal-original-single-levels_masked','seasonal-original-single-levels','seasonal-original-single-levels','seasonal-original-single-levels','seasonal-original-single-levels','seasonal-original-single-levels'],['seasonal-original-single-levels_derived','seasonal-original-single-levels_derived','seasonal-original-single-levels_masked','seasonal-original-single-levels','seasonal-original-single-levels','seasonal-original-single-levels','seasonal-original-single-levels','seasonal-original-single-levels']] ##as time_name but for the name of the head folder containing hindcasts and predictions
