...
for vv in np.arange(len(variable_fc)):
    ...
    #filename_forecast = path_gcm_base+'/'+product+'/'+variable_fc[vv]+'/'+model+'/'+version+'/'+str(year_init)+str(month_init).zfill(2)+'/'+file_start[vv]+'_'+domain+'_'+product+'_'+variable_fc[vv]+'_'+model+'_'+version+'_'+str(year_init)+str(month_init).zfill(2)+'.nc'
...
savename = dir_forecast+'/terciles_'+model+version+'_init_'+str(year_init)+str(month_init).zfill(2)+'_'+str(season_length)+'mon_dtr_'+detrended+'_refyears_'+str(years_quantile[0])+'_'+str(years_quantile[1])+'.nc'
...
print('INFO: pred2tercile.py has been run successfully ! The netcdf output file has been stored at '+dir_forecast)
quit()
