...
quantile_version = 'v1l' #version of the validation results
agg_label = ['1mon','3mon'] # ['1mon','3mon'] list of strings used to label to aggregation period in the output files generated by this script; the length of this list defines the number of aggregation windows
...
for ag in np.arange(len(agg_label)):
    ...
    for mm in np.arange(len(model)):
        ...
        out_arr = np.expand_dims(out_arr,axis=0)
        out_arr = xr.DataArray(out_arr, coords=[[agg_label[ag]],[model[mm]],date_init,variable_std,np.array((1,2,3)),season_label,nc_fc[lat_name[-1]],nc_fc[lon_name[-1]]], dims=['aggregation','model','rtime','variable','tercile','season','y','x'], name='probability')
