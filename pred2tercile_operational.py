...
model = ['eccc','cmcc','ecmwf'] # ['cmcc','ecmwf'] models to be assessed
version = ['5','35','51'] # version of these models
...
exec(open(rundir+'/config/config_for_pred2tercile_operational.py').read()) #reads additional input variables (or arguments) used in this script
...
variable_std = [variable_std_eccc5,variable_std_cmcc35,variable_std_ecmwf51] # standard model variable names used by the pySeasonal package. Here used to load the quantile threshold files and to write the output netCDF files generated by this script
variable_fc = [variable_fc_eccc5,variable_fc_cmcc35,variable_fc_ecmwf51] # variable name used in the file name, i.e. outside the file, ask collegues for data format harmonization
variable_fc_nc = [variable_fc_nc_eccc5,variable_fc_nc_cmcc35,variable_fc_nc_ecmwf51] # variable name within the model netcdf file, may vary depending on source
time_name = [time_name_eccc5,time_name_cmcc35,time_name_ecmwf51] #name of the time dimension within the model netcdf file, may vary depending on source
lon_name = [lon_name_eccc5,lon_name_cmcc35,lon_name_ecmwf51]
lat_name = [lat_name_eccc5,lat_name_cmcc35,lat_name_ecmwf51]
file_start = [file_start_eccc5,file_start_cmcc35,file_start_ecmwf51] #start string of the file names
    ...
    for mm in np.arange(len(model)):
        ...
        for vv in np.arange(len(variable_fc[mm])):
            ...
            if variable_std[mm][vv] in masked_variable_std:
                ...
                ValueError('check whether <variable_std[mm][vv]> is in <masked_variable_std> !')
