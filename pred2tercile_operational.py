...
for mm in np.arange(len(model)):
    ...
    filename_quantiles = dir_quantile+'/'+quantile_version+'/quantiles_pticlima_'+model[mm]+version[mm]+'_'+domain+'_'+str(years_quantile[mm][0])+'_'+str(years_quantile[mm][1])+'_'+quantile_version+'.nc'
    ...
    for vv in np.arange(len(variable_fc)):
        ...
        nc_fc, file_valid = transform_gcm_variable(nc_fc,variable_fc[vv],variable_qn[vv],model[mm],version[mm])
        ...
        for mo in season_start_month:
            ...
            if variable_fc[vv] == 'pr':
                print('INFO: setting seasonal mean '+variable_fc[vv]+' values from '+model[mm]+ '< '+str(precip_threshold)+' to 0...')
