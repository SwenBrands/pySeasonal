...
#load model specific variables to be processed: variable names, time, lon and lat names, as well as start of the input file names; note that the model and version order within the sublists in the following 7 lines must agree with the order indicated in <model> and <version> above 
variable_std = []
variable_fc = []
variable_fc_nc = []
time_name = []
...
for mm in range(len(model)):
    variable_std.append(config[model[mm]+version[mm]]['variable_std']) # standard model variable names used by the pySeasonal package. Here used to load the quantile threshold files and to write the output netCDF files generated by this script
    variable_fc.append(config[model[mm]+version[mm]]['variable_fc'])
    variable_fc_nc.append(config[model[mm]+version[mm]]['variable_fc_nc'])
    time_name.append(config[model[mm]+version[mm]]['time_name'])
    lon_name.append(config[model[mm]+version[mm]]['lon_name'])
    lat_name.append(config[model[mm]+version[mm]]['lat_name'])
    file_start.append(config[model[mm]+version[mm]]['file_start'])
