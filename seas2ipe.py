...
masked_variables_std = ['pvpot','fwi','SPEI-3-M'] #list of variables on which a land-sea mask will be applied, setting values over sea to nan
...
for ag in np.arange(len(agg_labels)):
    ...
    for mm in np.arange(len(model)):
        ...
        for vv in np.arange(len(variables_std)):
            ...
            #replace nan data wit -1e20
            valid_data_bool = ~np.isnan(nc_forecast_step_prob)
            nc_forecast_step_prob = nc_forecast_step_prob.where(valid_data_bool, other=-1e20)
            ...
            nc_forecast_step_argmax = nc_forecast_step_prob.argmax(dim='tercile').astype(datatype)+1
            ...
            if variables_std[vv] in masked_variables_std:
                ...
                mask_appended = np.tile(nc_mask.mask,(len(nc_forecast_step_argmax.rtime),len(nc_forecast_step_argmax.season),1,1))
                nc_forecast_step_argmax = nc_forecast_step_argmax.where(mask_appended == 1, other=np.nan)
                nc_forecast_step_maxprob = nc_forecast_step_maxprob.where(mask_appended == 1, other=np.nan)
                nc_mask.close()
                del(nc_mask)
