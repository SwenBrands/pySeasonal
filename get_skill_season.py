...
if len(sys.argv) > 1:
    ...
    model = [str(sys.argv[2])] #list format
    agg_label = [str(sys.argv[3])] #list format
    modulator = [str(sys.argv[4])] #list format
    phase = [str(sys.argv[5])] #list format
else:
    ...
    model = ['eccc5','cmcc35','ecmwf51'] #['cmcc35','ecmwf51'] list of models and versions thereof
    ...
    phase = ['none','0','1','2'] #changed to string format for all entries in the new version

    # vers = 'v1m_test' #version number of the output netCDF file to be sent to Predictia; will be extended by "_mon" or "_seas" depending on whether 1-month or 3-month values will be verified
    # model = ['eccc5'] #['cmcc35','ecmwf51'] list of models and versions thereof
    # agg_label = ['3mon'] #list of strings used to label to aggregation period in the output files generated by this script; the length of this list defines the number of aggregation windows
    # modulator = ['enso'] # list containing modulators climate oscillation assumed to modulate the verification results; currently: "enso" or "none"
    # phase = ['1'] # enso phase the validation is condition on: 0 = neutral, 1 = El Niño, 2 = La Niña; 'none' indicates that none is used; must have the same length as <modulator>
...
variables_gcm = ['t2m','tp','msl','si10','ssrd'] #model variable names in CDS format  GCM variable names have been set to ERA5 variable names from CDS in <aggregate_hindcast.py> except for <SPEI-3-M> and <SPEI-3-R>, which are paired with <SPEI-3> in <variables_obs>)
variables_obs = ['t2m','tp','msl','si10','ssrd'] #variable names in observations; are identical to <variables_gcm> except for <SPEI-3>, which is referred to as <SPEI-3-M> or <SPEI-3-R> in the model depending on whether past values are taken from the model or reanalysis (i.e. quasi-observations)
...
exec(open(rundir+'/config/config_for_get_skill_season.py').read()) #reads the <functions_seasonal.py> script containing a set of custom functions needed here

#load the season configurations depending on the aggregation periods specified in <agg_label>
season_label = []
season = []
for ag in np.arange(len(agg_label)):
    season_label_step = eval(eval("'season_label_'+seas", {"seas": agg_label[ag]}))
    season_step = eval(eval("'season_'+seas", {"seas": agg_label[ag]}))
...
for mm in np.arange(len(model)):
    nr_mem_step = eval(eval("'nr_mem_'+mod", {"mod": model[mm]}))
    years_model_step = eval(eval("'years_'+mod", {"mod": model[mm]}))
    nr_mem.append(nr_mem_step)
    years_model.append(years_model_step)
    
    leads_per_model = [] #init lead-times per model
    for ag in np.arange(len(agg_label)):
        lead_step = eval(eval("'lead_'+mod+'_'+aggr", {"mod": model[mm],"aggr": agg_label[ag]}))
                    ...
                    for oo in np.arange(len(obs)):
                        ...
                        for ll in np.arange(len(lead_label)):
                            if modulator[mo] == 'enso':
                                ...
                                phase_ind = (indices_sub.oni2enso == int(phase[mo])).values
...
print('INFO: get_skill_season.py has been run successfully. A flag is written at '+dir_flag)
flagfile = dir_flag+'/get_skill_season_'+str(vers)+'_'+str(model)+'_'+str(agg_label)+'_'+str(modulator)+'_'+str(phase)+'_.flag'
flagfile.replace("[","").replace("]","").replace("'","").replace(",","_")
file = open(flagfile,'w')
file.write('get_skill_season.py has been run successfully for '+str(vers)+', '+str(model)+', '+str(agg_label)+', '+str(modulator)+', '+str(phase))
file.close()
quit()