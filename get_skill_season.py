...
if len(sys.argv) > 1:
    ...
    elif variables_gcm[0] in ('TR-C4','Rx1day-C4','Rx5day-C4','TXm-C4','FD-C4','SU-C4','SPEI-3-M-C4','FWI-C4'):
        delete_me = variables_gcm[0][-3:]
        if delete_me == '-C4':
            print(delete_me+' will be removed from '+variables_gcm[0]+' to construct the name of <variables_obs> !')
            variables_obs = [variables_gcm[0][0:-3]] #removes "-C4" for observed variables
        else:
            raise Error('Unknown string sequence '+delete_me+' assigned for removal from variables_gcm[0] to construct variables_obs !')        
    ...
    vers = 'v1q' #version number of the output netCDF file to be sent to Predictia; will be extended by "_mon" or "_seas" depending on whether 1-month or 3-month values will be verified
    models = ['ecmwf51'] #['cmcc35','ecmwf51'] list of models and versions thereof
    domain = 'Canarias' # string describing a single domain
    ...
    # variables_gcm = ['TR-C4'] #model variable names in CDS format  GCM variable names have been set to ERA5 variable names from CDS in <aggregate_hindcast.py> except for <SPEI-3-M> and <SPEI-3-R>, which are paired with <SPEI-3> in <variables_obs>)
