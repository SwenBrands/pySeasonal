...
configuration_file = 'config_for_get_skill_season_Canarias.yaml'
# configuration_file = 'config_for_get_skill_season_medcof.yaml'
...
if len(sys.argv) > 1:
    ...
    variables_gcm = ['FD-C4','SU-C4','TR-C4'] #model variable names in CDS format  GCM variable names have been set to ERA5 variable names from CDS in <aggregate_hindcast.py> except for <SPEI-3-M> and <SPEI-3-R>, which are paired with <SPEI-3> in <variables_obs>)
    variables_obs = ['FD','SU','TR'] #variable names in observations; are identical to <variables_gcm> except for <SPEI-3>, which is referred to as <SPEI-3-M> or <SPEI-3-R> in the model depending on whether past values are taken from the model or reanalysis (i.e. quasi-observations)
    agg_labels = ['1mon','5mon'] #list of strings used to label to aggregation period in the output files generated by this script; the length of this list defines the number of aggregation windows
                ...
                for vv in np.arange(len(variables_gcm)):       
                    ...
                    for oo in np.arange(len(obs)):
                        ...
                        for ll in np.arange(len(lead_label)):
                            ...
                            pearson_r_mod.close()
                            rpc.close()
                            del(phase_ind,pearson_r,pearson_pval,pearson_pval_effn,spearman_r,spearman_pval,spearman_pval_effn,bias,relbias,crps_ensemble, \
                            reliability_lower,reliability_center,reliability_upper,roc_auc_lower,roc_auc_center,roc_auc_upper,pearson_r_mod,rpc)
                        ...
                        reliability_center = xr.DataArray(reliability_center_np, coords=[obs_seas_mn_5d.season,obs_seas_mn_5d.lead,obs_seas_mn_5d.y,obs_seas_mn_5d.x], dims=['season','lead','y','x'], name='reliability_center_tercile')
                        ...
                        roc_auc_center = xr.DataArray(roc_auc_center_np, coords=[obs_seas_mn_5d.season,obs_seas_mn_5d.lead,obs_seas_mn_5d.y,obs_seas_mn_5d.x], dims=['season','lead','y','x'], name='roc_auc_center_tercile')
                        ...
                        results = xr.merge((pearson_r,pearson_pval,pearson_pval_effn,spearman_r,spearman_pval,spearman_pval_effn,bias,relbias,crps_ensemble,crps_ensemble_skillscore,reliability_lower,reliability_center,reliability_upper,roc_auc_lower,roc_auc_center,roc_auc_upper,roc_auc_lower_skillscore,roc_auc_center_skillscore,roc_auc_upper_skillscore,rpc)) #merge xr dataarrays into a single xr dataset
                        ...
                        rpc.close()
                        indices_sub.close()
                        del(results,pearson_r,pearson_pval,pearson_pval_effn,spearman_r,spearman_pval,spearman_pval_effn,bias,relbias,crps_ensemble,crps_ensemble_skillscore,obs_seas_mn_5d,obs_seas_mn_5d_nw,gcm_seas_mn_5d,gcm_seas_mn_6d,gcm_seas_mn_6d_nw,reliability_lower,reliability_center,reliability_upper,roc_auc_lower,roc_auc_center,roc_auc_upper,roc_auc_lower_skillscore,roc_auc_center_skillscore,roc_auc_upper_skillscore,rpc,indices_sub)
