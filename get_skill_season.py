...
if len(sys.argv) > 1:
    ...
    domain_for_config = [str(sys.argv[3])] #list format
    ...
    FLAGDIR = str(sys.argv[8]) #string containing the path the flag directory
    ...
    # domain_for_config = 'Canarias' # string describing a single domain
    ...
    # models = ['ecmwf51'] #['cmcc35','ecmwf51'] list of models and versions thereof
    # domain_for_config = 'Canarias' # string describing a single domain
    # variables_gcm = ['SU-C4'] #model variable names in CDS format  GCM variable names have been set to ERA5 variable names from CDS in <aggregate_hindcast.py> except for <SPEI-3-M> and <SPEI-3-R>, which are paired with <SPEI-3> in <variables_obs>)
    # variables_obs = ['SU'] #variable names in observations; are identical to <variables_gcm> except for <SPEI-3>, which is referred to as <SPEI-3-M> or <SPEI-3-R> in the model depending on whether past values are taken from the model or reanalysis (i.e. quasi-observations)
    ...
    vers = 'v_test' #version number of the output netCDF file to be sent to Predictia; will be extended by "_mon" or "_seas" depending on whether 1-month or 3-month values will be verified
    models = ['ecmwf51'] #['cmcc35','ecmwf51'] list of models and versions thereof
    domain_for_config = 'medcof' # string describing a single domain
    variables_gcm = ['SPEI-3-M'] #model variable names in CDS format  GCM variable names have been set to ERA5 variable names from CDS in <aggregate_hindcast.py> except for <SPEI-3-M> and <SPEI-3-R>, which are paired with <SPEI-3> in <variables_obs>)
    variables_obs = ['SPEI-3'] #variable names in observations; are identical to <variables_gcm> except for <SPEI-3>, which is referred to as <SPEI-3-M> or <SPEI-3-R> in the model depending on whether past values are taken from the model or reanalysis (i.e. quasi-observations)
    agg_labels = ['5mon'] #list of strings used to label to aggregation period in the output files generated by this script; the length of this list defines the number of aggregation windows
    modulators = ['none'] # list containing modulators climate oscillation assumed to modulate the verification results; currently: "enso" or "none"
    phases = ['none'] #changed to string format for all entries in the new version
    FLAGDIR = '/lustre/gmeteo/PTICLIMA/Scripts/SBrands/pyPTIclima/pySeasonal/FLAG/get_skill'
...
configuration_file = 'config_for_get_skill_season_'+domain_for_config+'.yaml'
...
for mo in np.arange(len(modulators)):
    for ag in np.arange(len(agg_labels)):
        ...
        dir_netcdf_scores = dir_netcdf+'/'+agg_labels[ag]+'/scores/'+domain
        dir_netcdf_tercilprobs = dir_netcdf+'/'+agg_labels[ag]+'/tercile_probabilities/'+domain
        dir_netcdf_quantiles = dir_netcdf+'/'+agg_labels[ag]+'/quantiles/'+domain
        ...
        for mm in np.arange(len(models)):
            for det in np.arange(len(detrending)):
                for vv in np.arange(len(variables_gcm)):       
                    ...
                    nc_gcm = xr.open_dataset(path_gcm, decode_timedelta=False)
                    #nc_gcm = xr.open_dataset(path_gcm, chunks = {'member' : 1}, decode_timedelta=False)
                    ...
                    for oo in np.arange(len(obs)):
                        ...
                        nc_obs = xr.open_dataset(path_obs, decode_timedelta=False)
            ...
            if modulators[mo] == 'none':
                ...
                if variables_gcm[vv] in masked_variables_std: # <masked_variables_std> is defined in ./config/config_for_get_skill_season.py
                    ...
                    lower_tercile_obs = apply_sea_mask(lower_tercile_obs,mask_file) #mask_file is defined above
